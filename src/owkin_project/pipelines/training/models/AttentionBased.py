...
class SimpleAttentionBlock(torch.nn.Module):
    """
    Simple Attention Block to be used in the AttentionModuleAggregator
    """
...
class GatedAttentionBlock(torch.nn.Module):
    """
    Updated version of the AttentionBlock using a gated attention mechanism to be used in the AttentionModuleAggregator. From the paper:
    Jiani Zhang et al. GaAN: Gated Attention Networks for Learning on Large and Spatiotemporal Graphs
    """
    def __init__(self, input_dim = 3, embed_dim = None):
...
def get_model(att_block = 'gated',
    ...
    """Main function to call to instanciate the model

    Args:
        att_block (str, optional): Which attention block to use, either 'gated' or 'simple'. Defaults to 'gated'.
        input_dim (int, optional): Dim of input features (embedding dim of images) . Defaults to 2048.
        agg_embed_dim (int, optional): Dim of the aggregated features. Defaults to 1024.
        cl_hidden_layers_size (list, optional): List of the hidden layer sizes in the classifier. Defaults to [512].
        transformers_first (bool, optional): If true, run a first self attention layer. Defaults to False.

    Returns:
        model: AttentionBased torch model reproducing Dual Stream Network
    """
